...
internals.authenticate = function (settings) {
    ...
    return function (name, options, callback) {
        ...
        return function authenticate(request, next) {
            ...
            if (!next) {
                next = function (err) {
                    var failureRedirect = (options.failureRedirect || (settings.urls && settings.urls.failureRedirect) || '/');
                    var successRedirect = (options.successRedirect || (settings.urls && settings.urls.successRedirect) || '/');

                    if (settings.apiMode) {
                        return err ? request.reply(Hapi.error.unauthorized('Unauthorized'))
                                   : request.reply().code(204).header('Location',successRedirect);
                    } else {
                        return request.reply.redirect(err ? (options.failureRedirect || (settings.urls && settings.urls.failureRedirect) || '/')
...
internals.Scheme.prototype.authenticate = function (request, callback) {
    ...
    if (this.settings.apiMode) {
        return callback(new Hapi.error.unauthorized('Unauthorized'));
    }
